...
IUSE="${IUSE_VIDEO_CARDS}
	bindist +classic debug +egl g3dvl +gallium gbm gles1 gles2 +llvm llvm-shared-libs
	mangling +nptl opencl openvg osmesa pax_kernel pic r600-llvm-compiler selinux
	+shared-glapi vdpau wayland xvmc xa xorg kernel_FreeBSD"

REQUIRED_USE="
	g3dvl?  ( gallium )
	llvm?   ( gallium )
	opencl? ( gallium llvm llvm-shared-libs )
	openvg? ( egl gallium )
	gbm?    ( shared-glapi )
	g3dvl? ( || ( vdpau xvmc ) )
	llvm-shared-libs? ( llvm )
	vdpau? ( g3dvl )
...
src_configure() {
	...
	if use opencl; then
		myconf+="
			--with-clang-libdir=/usr/$(get_libdir)
		"
	fi
...
src_install() {
	...
	if ! tc-is-cross-compiler; then
		newbin "${S}"/src/glsl/builtin_compiler/builtin_compiler glsl_compiler
	fi
