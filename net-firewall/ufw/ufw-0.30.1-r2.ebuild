...
pkg_postinst() {
	...
	if path_exists -o "${EROOT}"lib/ufw/user{,6}.rules; then
		elog "Attention!"
		elog "User configuration from /lib/ufw is now placed in /etc/ufw/user."
		# quoting to avoid "Unquoted Variable" warning
		elog "Please stop ufw, copy .rules files from ""${EROOT}""lib/ufw" \
