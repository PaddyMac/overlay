...
pkg_postinst() {
	elog "PLEASE NOTE:"
	elog ""
	elog "In order to use OSS v${MY_PV}-build${MY_BUILD} you must run"
	elog "# /etc/init.d/oss start "
	elog ""
	elog "If you are upgrading from a previous build of OSS v4.x you must run"
	elog "# /etc/init.d/oss restart "
	elog ""
	elog "Enjoy OSS v${MY_PV}-build${MY_BUILD} !"
}
